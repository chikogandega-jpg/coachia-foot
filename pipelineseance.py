...
def construire_schema_tactique(formation: str) -> str:
    """Génère les positions des joueurs sur le terrain à partir d'une
    formation type '4-3-3', '4-4-2', '3-5-2', etc. Le gardien est ajouté
    automatiquement en plus des lignes indiquées."""
    try:
        lignes_effectifs = [int(n) for n in formation.split("-") if n.strip().isdigit()]
    except Exception:
        lignes_effectifs = [4, 3, 3]

    if not lignes_effectifs:
        lignes_effectifs = [4, 3, 3]

    bandes = [1] + lignes_effectifs  # gardien + lignes du terrain
    nb_bandes = len(bandes)

    joueurs_html = []
    numero = 2  # le gardien porte le numéro 1

    for i, effectif_ligne in enumerate(bandes):
        # y=92% près du gardien (bas), y=8% près de l'attaque (haut)
        y_pct = 92 - i * (84 / max(nb_bandes - 1, 1))

        for j in range(effectif_ligne):
            if effectif_ligne == 1:
                x_pct = 50
            else:
                x_pct = 12 + j * (76 / (effectif_ligne - 1))

            label = "1" if i == 0 else str(numero)
            joueurs_html.append(
                f'<div class="joueur" style="left:{x_pct:.0f}%; top:{y_pct:.0f}%;">{label}</div>'
            )
            if i != 0:
                numero += 1
...
def generer_pdf(club: dict, contenu_seance: dict, contexte_semaine: dict) -> Path:
    ...
    remplacements = {
        ...
        "{{schema_tactique}}": construire_schema_tactique(club["formation"] or "4-3-3"),
